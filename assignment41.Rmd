...
mean.data.normalize <- (mean.data - min(mean.data))/(max(mean.data) - min(mean.data))
classDigit.mean.matrix <- matrix(abs(mean.data.normalize),28,28, byrow = T)
...
#The Principal Components (or Eigenvectors) basically seek directions in which it is more efficient to  represent the data. This is particularly useful for reducing the computational effort. it scales tha data to mean and calculates principal components
...
# weight calculation using mean data
 ...
 image.reconstruct <- function(eigenno, imageno) {
  ...
  img.normalize <- (img - min(img))/(max(img) - min(img))
...
writeJPEG(matrix(as.numeric(classDigit[which.min(d5k),])/255, 28, 28, byrow = T), target = "check4.jpeg")
writeJPEG(matrix(as.numeric(classDigit[which.min(d6k),])/255, 28, 28, byrow = T), target = "check4.jpeg")
...
pls1.fit<-plsr(log(housing.data.test$SalePrice) ~ MSSubClass + 
    MSZoning + LotArea + LotConfig + Neighborhood + Condition1 + 
    BldgType + OverallQual + OverallCond + YearBuilt + RoofStyle + 
    Exterior1st + ExterQual + ExterCond + Foundation + BsmtExposure + 
    BsmtFinType1 + BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF + HeatingQC + 
    CentralAir + Electrical + X1stFlrSF + X2ndFlrSF + LowQualFinSF + 
    BsmtFullBath + FullBath + BedroomAbvGr + KitchenAbvGr + Functional + 
    Fireplaces + FireplaceQu + GarageCars + GarageArea + GarageCond + 
    WoodDeckSF + OpenPorchSF + EncPorchSF + PoolArea, data = housing.data.test, method = "kernelpls",validation = "CV",8)
...
CARET.TRAIN.CTRL <- trainControl(method="repeatedcv",
                                 ...
                                 repeats=5,
                                 verboseIter=FALSE)
housing.data.train[,cateogerical.var] <- sapply(housing.data.train[,cateogerical.var], as.numeric)
...
# print(length(pred))
# prediction <- as.data.frame(as.matrix(pred))
# colnames(prediction) <- "SalePrice"
# submission <- data.frame(Id = test$Id, SalePrice = prediction)
